...
class KwikDeliveryController extends Controller
{
    ...
    public function calculatePricing($deliveryAddress, $deliveryLatitude, $deliveryLongitude, Product $product, User $seller, User $user, $checkOnly = false)
    {
        $task = [
            ...
            "deliveries"                   => [
                $this->buildDeliveryPayload($user, $deliveryAddress, $deliveryLatitude, $deliveryLongitude),
            ],
            "has_pickup"                   => 1,
            "has_delivery"                 => 1,
            "auto_assignment"              => 1,
            "user_id"                      => 1,
            "pickups"                      => [
                $this->buildPickupPayload($product, $seller),
            ],
            "payment_method"               => 32,
            "form_id"                      => 2,
        ...
        try {
            ...
            if (isset($response['data']['per_task_cost'])) {
                ...
                // Build a unique cache key per user + product + delivery address + coordinates
                $cacheKey = 'kwik_pricing_' . md5($user->id . '_' . $product->id . '_' . $deliveryAddress);

                // Cache the response for 4 minutes (240 seconds)
                $cachedValue = Cache::remember($cacheKey, 240, function () use ($response) {
            ...
            $errorMessage = $response['message'] ?? 'Unknown error contacting Kwik API';
            Log::error("Kwik Delivery API error: " . $errorMessage);

            return ['error' => $errorMessage];
        } catch (\Exception $e) {
            // Handle exceptions (network issues, timeouts, etc.)
            Log::error("Kwik Delivery API exception: " . $e->getMessage(), ['exception' => $e]);

            return ['error' => 'An error occurred while contacting Kwik API. Please try again later.'];
        }
    }

    protected function buildDeliveryPayload(User $user, $deliveryAddress, $deliveryLatitude, $deliveryLongitude)
    {
        return [
            'address'         => $deliveryAddress,
            'email'           => $user->email ?? '',
    ...
    protected function buildPickupPayload(Product $product, User $seller)
    {
        return [
            'address'   => $product->product_location ?? '',
            'email'     => $seller->email ?? '',
